...
-- Full-Screen a window on the other monitor
hs.hotkey.bind({"cmd", "alt", "ctrl"}, "Up", function()
  local win = hs.window.focusedWindow()

  if win:screen() == hs.screen.allScreens()[1] then
    targetWindow = hs.screen.allScreens()[2]
  else
    targetWindow = hs.screen.allScreens()[1]
  end

  win:moveToScreen(targetWindow)
...
start = function(time)-- {{{1
    if isActive then return end

    if time then timeLeft = time * 60 end
...
hs.hotkey.bind(super, hotkey, function() start(25) end)
hs.hotkey.bind(super, hotkeyBreak, function() start(5) end)
